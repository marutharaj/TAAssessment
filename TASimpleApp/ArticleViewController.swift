...
class ArticleViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    @IBAction func showSearchOption(sender: AnyObject) {
        ...
        alert.addAction(UIAlertAction(title: "7", style: .default , handler:{ (UIAlertAction)in
            self.showActivityIndicator()
            ArticleService().sendRequest(period: 7, completionHandler: { (articles) in
                // here is your article object
                self.articles = articles
        ...
        alert.addAction(UIAlertAction(title: "30", style: .default , handler:{ (UIAlertAction)in
            self.showActivityIndicator()
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if(article.title.count > 40) {
            let index = article.title.index(article.title.startIndex, offsetBy: 30)
        ...
        cell.articlePublishedDateLabel.text = TAUtility().stringFromDate(date:article.publishedDate)
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if (segue.identifier == "articleDetailSegueIdentifier") {
            ...
            viewController.article = self.articles[cell.indexPath!]
        }
    }
